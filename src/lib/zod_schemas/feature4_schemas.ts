...
export const F4B = {
  ...
  createQuest: z.object({
    ...
    p_reward_crystal: z.number().int().min(0).max(10000),
    p_duration_minutes: z.number().int().min(1).max(1440),
  }).refine(v => v.p_reward_gold > 0 || v.p_reward_bv > 0 || v.p_reward_crystal > 0, { message: '보상은 GOLD, CRYSTAL, BV 중 하나 이상이어야 합니다' }),
