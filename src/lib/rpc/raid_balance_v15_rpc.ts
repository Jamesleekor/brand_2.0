...
export interface RaidDeleteRecordResult {
  deleted: boolean;
  raid_id: number;
  title: string;
  attack_batch_count: number;
  participant_snapshot_count: number;
  reward_settlement_count: number;
  reward_reversal_applied: boolean;
}
...
export const raidBalanceV15Rpc = {
  ...
  deleteRecord: (supabase: SupabaseClient, raidId: number) =>
    callRpc<RaidDeleteRecordResult>(supabase, 'teacher_delete_raid_record', {
      p_raid_id: raidId,
    }),
};
