    ...
    <>
      ...
      >
        ...
        <div className="space-y-10 px-4 py-8 sm:px-7 sm:py-10 lg:px-9">
          ...
          <div className="rounded-[18px] border border-sky-100/14 bg-sky-100/[0.03] px-4 py-3 text-center text-sm font-semibold leading-6 text-slate-100/82">
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-4 sm:gap-5">
          ...
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm font-bold leading-7 text-slate-100/82 sm:text-base [word-break:keep-all]">
        ...
        <div className="w-fit rounded-full border border-sky-100/18 bg-black/18 px-4 py-2 text-center">
          ...
          <div className="text-[10px] font-black tracking-[0.12em] text-slate-100/78">성좌 기록</div>
        </div>
      </div>
    </header>
  );
  ...
  return (
    <section className="space-y-5">
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="mx-auto mt-2 max-w-3xl text-sm font-semibold leading-7 text-slate-100/80 [word-break:keep-all]">{description}</p>
      </div>
...
function ConstellationCore({ entry, tone, tiedRows }: { entry: HallOfGloryEntry; tone: ConstellationTone; tiedRows: HallOfGloryEntry[] }) {
  ...
  return (
    <div className={`relative flex min-h-[260px] flex-col items-center justify-center rounded-[24px] border border-white/12 bg-black/22 px-5 py-6 text-center ${glow}`}>
      ...
      <div className="mt-4 text-[10px] font-black tracking-[0.18em] text-slate-100/78">성좌의 중심</div>
      ...
      {entry.subject_brand_name && <div className="mt-1 text-sm font-bold text-slate-100/78">{entry.subject_brand_name}</div>}
      <div className={`mt-4 whitespace-nowrap font-display text-4xl ${accent}`}>
        {formatNumber(entry.value_primary ?? 0)}<span className="ml-1.5 text-base text-slate-100/84">개</span>
      </div>
      <div className="mt-4"><RecordStatusBadge /></div>

      {tiedRows.length > 0 && (
        <div className="mt-4 w-full border-t border-white/10 pt-3">
          <div className="text-[10px] font-black tracking-[0.12em] text-slate-100/74">공동 기록 보유자</div>
...
function AchievementAtlas({ achievements, tone }: { achievements: HallAchievementDetail[]; tone: ConstellationTone }) {
  if (achievements.length === 0) {
    return (
      <div className="flex min-h-[260px] items-center justify-center rounded-[24px] border border-white/10 bg-black/14 px-5 text-center text-sm font-semibold text-slate-100/78">
        세부 업적 목록을 불러오는 중입니다.
      </div>
    );
  }
