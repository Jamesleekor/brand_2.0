...
function RecordGroup({ rows }: { rows: HallOfGloryEntry[] }) {
  ...
  const hasLiveDerived = rows.some((row) => row.source_kind === 'PRODUCTION_DERIVED');
  const hasHistorical = rows.some((row) => row.source_kind !== 'PRODUCTION_DERIVED');
    ...
    <div className="rounded-card-md border border-line bg-bg-deep/70 overflow-hidden">
      <div className="px-3 py-2.5 border-b border-line">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <HallRecordBadge live={hasLiveDerived} mixed={hasLiveDerived && hasHistorical} compact />
        </div>
            ...
            <div className="min-w-0 flex items-center gap-2">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-text-muted font-bold truncate">
                  ...
                  {entry.source_kind === 'PRODUCTION_DERIVED' ? ' · 현재 공식' : ''}
                </div>
              </div>
            </div>
...
function RecordCard({ entry }: { entry: HallOfGloryEntry }) {
  ...
  const live = entry.source_kind === 'PRODUCTION_DERIVED';
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-1.5">
            <HallRecordBadge live={live} compact />
            {(entry.period_label || entry.season_label || entry.school_year) && (
...
function HallRecordBadge({ live, mixed = false, compact = false }: { live: boolean; mixed?: boolean; compact?: boolean }) {
  const label = mixed ? '현재·역대 통합' : live ? '현재 공식 기록' : '공식 확정 기록';
  const tone = live ? 'border-bv/30 bg-bv/10 text-bv' : 'border-gold/30 bg-gold/10 text-gold';
  return (
    <span className={`rounded-pill border font-black ${tone} ${compact ? 'px-2 py-0.5 text-[9px]' : 'px-2.5 py-1 text-2xs'}`}>
      {label}
    </span>
  );
}
