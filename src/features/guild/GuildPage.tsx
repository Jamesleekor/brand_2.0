...
function GuildHeader({data,contribution}:{data:any;contribution:any}){
  const g=data.guild,meta=elementMeta(g.my_element),summary=contribution?.summary,row=contribution?.contribution;
  return <section className="relative overflow-hidden glass-card p-5"><div className="absolute inset-0 bg-gradient-to-br from-brand-primary/15 via-transparent to-gold/10 pointer-events-none"/><div className="relative flex gap-4 items-start"><div className="w-16 h-16 rounded-card-lg overflow-hidden bg-bg-deep border border-line-brand flex items-center justify-center text-3xl flex-shrink-0">{g.logo_url?<img src={g.logo_url} alt="길드 문장" className="w-full h-full object-cover"/>:'⚔️'}</div><div className="flex-1 min-w-0"><div className="text-xs text-text-secondary font-black">{data.activeSeason?.display_name||'활성 시즌 미설정'}</div><h2 className="font-display text-2xl text-white mt-0.5 truncate">{g.name}</h2><p className="text-sm text-text-secondary mt-1">{g.slogan||g.description||'우리 길드'}</p><div className="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-pill bg-bg-deep border border-line text-xs font-black text-bv"><span>{meta.emoji}</span><span>내 담당 속성 · {meta.label}</span></div></div></div><div className="relative grid grid-cols-2 lg:grid-cols-4 gap-2 mt-4"><ScoreBox label={`${Number(data.currentMonth.slice(5))}월 길드 GS`} value={summary?formatNumber(Number(summary.draft_gs_total)):'집계 전'} sub={summary?.draft_rank?`초안 ${summary.draft_rank}위`:'Guild 2A 초안'}/><ScoreBox label="길드 GS 확정" value="준비 중" sub="Guild 5 월 마감에서 확정" small/><ScoreBox label="길드원" value={`${data.members.length}명`} sub="현재 활성 소속 기준"/><ScoreBox label="개인 기여도" value={row?`${formatNumber(Number(row.final_total))}점`:'집계 대기'} sub={row?`기본 ${formatNumber(Number(row.basic_total))} / 900`:'Guild 2 계산 후 표시'} small/></div></section>;
}
...
function ContributionCard({data,isLoading,error}:{data:any;isLoading:boolean;error:unknown}){
  ...
  if(!row)return <section className="glass-card p-4 border-brand-primary/20"><div className="font-display text-lg">✨ 이달의 개인 기여도</div><p className="mt-2 text-sm text-text-secondary">아직 이번 달 점수 초안이 만들어지지 않았어요. 길드 운영자가 점수를 계산하면 여기에서 확인할 수 있습니다.</p></section>;
  const categories=Object.entries(row.calculation_metadata?.observation_category_counts??{}) as [string,unknown][];
  const card=(label:string,value:number,max:number,status:string,description:string)=><div className="rounded-card-md border border-line bg-bg-deep p-3"><div className="text-xs font-black text-text-secondary">{label}</div><div className="mt-1 font-display text-xl text-gold">{formatNumber(value)} <span className="text-xs">/ {max}</span></div><div className={`mt-1 text-[10px] font-black ${status==='READY'?'text-success':status==='PENDING'?'text-warning':'text-text-muted'}`}>{CONTRIBUTION_STATUS[status]||status}</div><p className="mt-1 text-[11px] leading-relaxed text-text-secondary">{description}</p></div>;
  return <section className="glass-card p-4"><div className="flex flex-wrap items-end justify-between gap-2"><div><h3 className="font-display text-lg">✨ {data.yearMonth.slice(5)}월 개인 기여도</h3><p className="mt-1 text-xs text-text-secondary">월 마감 전의 초안입니다. 최종 확정은 Guild 5에서 진행됩니다.</p></div><div className="text-right"><div className="font-display text-2xl text-gold">{formatNumber(Number(row.final_total))}점</div><div className="text-xs text-text-secondary">기본 {formatNumber(Number(row.basic_total))} / 900 · 아케이드 +{formatNumber(Number(row.arcade_applied))} / 90</div></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">{card('동료평가',Number(row.peer_points),300,row.peer_status,'길드원들의 평가를 종합·보정하여 반영합니다.')}{card('미션 기여',Number(row.mission_points),300,row.mission_status,'미션 기능이 연결되면 활동 기록과 교사 등급을 반영합니다.')}{card('길드 세션',Number(row.session_points),150,row.session_status,`일반 불참 ${Number(row.session_absent_count)}회 × -30${Number(row.session_unmarked_count)>0?` · 미기록 ${Number(row.session_unmarked_count)}명`:''}`)}{card('길드 기여 기록',Number(row.teacher_observation_points),150,row.teacher_observation_status,`인정된 행동 ${Number(row.observation_count)}건 · 1건 +10점`)}{card('아케이드 보너스',Number(row.arcade_applied),90,row.arcade_status,`게임별 원점수 ${formatNumber(Number(row.arcade_raw_total))}점은 최대 +90점까지만 반영됩니다.`)}</div><div className="mt-4 grid gap-3 lg:grid-cols-2"><div className="rounded-card-md border border-line bg-bg-deep p-3"><div className="text-xs font-black text-text-secondary">기여 기록 분포</div>{categories.length?<div className="mt-2 flex flex-wrap gap-1.5">{categories.map(([category,count])=><span key={category} className="rounded-pill border border-line bg-bg-card px-2 py-1 text-xs text-text-secondary">{OBSERVATION_CATEGORY[category]||category} ×{String(count)}</span>)}</div>:<p className="mt-2 text-xs text-text-secondary">아직 인정된 기여 기록이 없습니다.</p>}</div><div className="rounded-card-md border border-line bg-bg-deep p-3"><div className="text-xs font-black text-text-secondary">학생에게 공개된 기여 메모</div>{!data.publicObservations.length?<p className="mt-2 text-xs text-text-secondary">공개된 메모가 없습니다. 비공개 메모는 교사만 볼 수 있습니다.</p>:<div className="mt-2 space-y-1.5">{data.publicObservations.map((event:any)=><div key={event.id} className="text-xs text-text-secondary"><b className="text-white">{OBSERVATION_CATEGORY[event.category]||event.category}</b> · {event.reason}</div>)}</div>}</div></div></section>;
}
