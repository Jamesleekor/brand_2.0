    ...
    <TeacherShell>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2"><Link className="btn-primary" to="/teacher/guild/scores">📊 Guild 2 점수</Link><Link className="btn-primary" to="/teacher/guild/missions">🗺️ Guild 3 미션</Link><button className="btn-secondary" disabled={!classroomId||healthLoading} onClick={async()=>{if(!classroomId)return;const r=await healthCall(()=>guildTeacherRpc.healthCheck(supabase,{p_classroom_id:classroomId}),{successTitle:'Guild 1 진단 완료'});if(r)setHealth(r);}}>🩺 {healthLoading?'진단 중...':'Guild 1 진단'}</button></div>
        </div>
