...
export function RaidHomeBanner() {
  ...
  const query = useQuery({
    queryKey: ['raid-portal-home', studentId],
  ...
  const raid = query.data?.active ?? null;
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-3">
        <div className="flex h-12 w-12 flex-none items-center justify-center rounded-card-md border border-white/20 bg-black/20 text-2xl shadow-[0_0_22px_rgba(34,211,238,0.12)]">
          {meta.icon}
        </div>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs font-black tracking-[0.16em] text-yellow-100">
              레이드 관문
            </span>
            {raid && (
      ...
      <div className="flex flex-none items-center gap-2">
        <span className="hidden text-xs font-black text-yellow-100 sm:inline">
          {raid?.status === 'ACTIVE' ? '전투 참가' : '관문 보기'}
        </span>
...
function bannerMeta(raid: RaidPortalRaid | null) {
  if (!raid) {
    return {
      icon: '🌀',
      description: '현재 개방된 레이드 없음',
      className:
        'border-cyan-300/28 bg-gradient-to-r from-cyan-950/75 via-slate-950/60 to-violet-950/65',
    };
  }
