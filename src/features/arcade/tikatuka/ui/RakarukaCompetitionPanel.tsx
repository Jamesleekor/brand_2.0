...
export function RakarukaCompetitionPanel({ periodId }: { periodId: number }) {
  ...
  return <section className="glass-card overflow-hidden border-gold/25 p-0">
    ...
    <div className="grid border-t border-line lg:grid-cols-2">
      ...
      <RankingTable title="🏅 공인 기록 랭킹 · 월간 보너스 기준" subtitle="1회 5판 · 도전 Lv 우선 · 동점 공동 순위 · Top 10 Guild 2 보너스">{data.official_leaderboard.length ? data.official_leaderboard.map(row => <div key={row.student_id} className="grid grid-cols-[48px_1fr_auto] items-center gap-3 border-b border-line/50 px-4 py-3 last:border-0"><RankBadge rank={row.rank}/><div><div className="font-black text-white">{row.brand_name || row.student_name}</div><div className="text-xs text-text-muted">{row.wins}승 {row.draws}무 {row.losses}패</div></div><div className="text-right"><div className="font-display text-lg text-gold">Lv.{row.difficulty}</div><div className="text-sm font-black text-success">{row.points}점</div></div></div>) : <EmptyRanking text="아직 성립한 공인 기록이 없습니다."/>}</RankingTable>
    </div>
  </section>;
}
