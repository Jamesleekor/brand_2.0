...
export default function NewbieSupportPage() {
  ...
  const [claimQuest, setClaimQuest] = useState<NewbieQuest | null>(null);
  ...
  const submitClaim = async () => {
    ...
    await call(() => newbieSupportRpc.submitClaim(supabase, {
      ...
      onSuccess: () => { setClaimQuest(null); refresh(); },
    });
  };
  ...
  return <div className="pb-10">
    ...
    <ClaimModal quest={claimQuest} mentors={mentors} evidenceId={evidenceId} setEvidenceId={setEvidenceId} mentorHelp={mentorHelp} setMentorHelp={setMentorHelp} note={note} setNote={setNote} loading={isLoading} onClose={() => setClaimQuest(null)} onSubmit={() => void submitClaim()} />
  </div>;
}
...
function QuestCard({ quest, onRequest, disabled }: { quest:NewbieQuest; onRequest:()=>void; disabled:boolean }) {
  ...
  const canRequest = done + quest.pending_count < target && quest.evidence_candidates.length > 0 && !disabled;
  return <div className="glass-card p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className="rounded-pill bg-brand-primary/15 px-2 py-0.5 text-[10px] font-black text-brand-glow">{done}/{target}</span><h3 className="font-black text-white">{quest.title}</h3></div><p className="mt-1 text-xs text-text-secondary">{quest.description}</p></div><button className="btn-secondary text-xs" disabled={!canRequest} onClick={onRequest}>{quest.pending_count>0?'검토 대기 중':done>=target?'완료':'완료 요청'}</button></div>
...
function ClaimModal({ quest, mentors, evidenceId, setEvidenceId, mentorHelp, setMentorHelp, note, setNote, loading, onClose, onSubmit }: {
  quest:NewbieQuest|null; mentors:ProgramMentor[]; evidenceId:number|null; setEvidenceId:(id:number|null)=>void;
  mentorHelp:Record<number,MentorHelpType>; setMentorHelp:(v:Record<number,MentorHelpType>)=>void; note:string; setNote:(v:string)=>void; loading:boolean; onClose:()=>void; onSubmit:()=>void;
}) {
  const selected = quest?.evidence_candidates.find((c:any) => Number(c.evidence_ref_id)===evidenceId) as any;
  return <Modal isOpen={!!quest} onClose={onClose} title={quest?.title ?? '정착 퀘스트'} emoji="🌱" size="lg"><div className="space-y-4">
    <div><label className="mb-1 block text-xs font-black text-text-secondary">완료 근거</label><select className="input-field w-full" value={evidenceId ?? ''} onChange={(e)=>setEvidenceId(e.target.value?Number(e.target.value):null)}><option value="">근거 선택</option>{quest?.evidence_candidates.map((c:any)=><option key={c.evidence_ref_id} value={c.evidence_ref_id}>{c.primary_label}{c.secondary_label?` · ${c.secondary_label}`:''}</option>)}</select>{selected && <div className="mt-2 rounded-card-md border border-line bg-bg-deep p-3 text-xs text-text-secondary"><div className="font-black text-white">{selected.primary_label}</div>{selected.secondary_label && <div>{selected.secondary_label}</div>}<div className="mt-1">{selected.summary}</div></div>}</div>
    ...
    <div className="flex justify-end gap-2"><button className="btn-secondary" onClick={onClose}>취소</button><button className="btn-primary" disabled={!evidenceId||loading} onClick={onSubmit}>완료 요청 보내기</button></div>
  </div></Modal>;
}
