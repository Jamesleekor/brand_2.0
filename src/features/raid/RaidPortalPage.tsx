...
export default function RaidPortalPage() {
  ...
  const [recentResultOpen, setRecentResultOpen] = useState(false);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#06101c] px-4 py-5 text-white md:px-6 md:py-7">
        ...
        <section className="mt-6 rounded-card-lg border border-white/15 bg-[#07111f]/78 p-5 backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {recent && (
              <button
                type="button"
                onClick={() => setRecentResultOpen(true)}
      ...
      {recent && (
        <RaidResultModal
          raidId={recent.id}
          open={recentResultOpen}
          onClose={() => setRecentResultOpen(false)}
        />
      )}
    </div>
  );
...
function ActiveRaidCard({
  ...
  const lobbyOpen = raid.status === 'LOBBY_OPEN';
    ...
    <div className="overflow-hidden rounded-card-lg border border-cyan-300/35 bg-[#07111f]/86 shadow-[0_0_40px_rgba(34,211,238,0.10)] backdrop-blur">
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="p-5 md:p-7">
          ...
          {!lobbyOpen && raid.boss_description && (
          ...
          {lobbyOpen ? (
            <div className="mt-5 grid max-w-xl grid-cols-2 gap-3">
              <div className="rounded-card-md border border-cyan-300/25 bg-cyan-500/5 p-3">
                <div className="text-[10px] font-black tracking-[0.15em] text-cyan-100">
                  속성
                </div>
              ...
              <div className="rounded-card-md border border-yellow-300/25 bg-yellow-400/5 p-3">
                <div className="text-[10px] font-black tracking-[0.15em] text-yellow-100">
                  전체 체력
                </div>
                <div className="mt-1 font-mono text-base font-black text-white">
                  {Number(raid.max_hp).toLocaleString('ko-KR')}
                </div>
              </div>
            </div>
          ) : (
        ...
        <div className="relative min-h-[260px] overflow-hidden border-t border-cyan-300/20 bg-black/35 lg:border-l lg:border-t-0">
          {lobbyOpen ? <BossSealedPanel raid={raid} /> : <RaidPreviewMedia raid={raid} />}
        </div>
      </div>
    </div>
  );
...
function BossSealedPanel({ raid }: { raid: RaidPortalRaid }) {
  return (
    <div className="flex min-h-[260px] h-full flex-col items-center justify-center px-6 text-center">
      <div className="flex h-20 w-20 items-center justify-center rounded-full border-2 border-cyan-300/30 bg-cyan-500/5 text-4xl shadow-[0_0_36px_rgba(34,211,238,0.12)]">
        🌀
      </div>
      <div className="mt-4 text-[10px] font-black tracking-[0.18em] text-cyan-100">
        관문 봉인 정보
      </div>
      <div className="mt-1 text-lg font-black text-white">{raid.boss_name}</div>
      <div className="mt-2 flex flex-wrap items-center justify-center gap-2">
        <span className="rounded-pill border border-white/20 bg-white/5 px-3 py-1 text-xs font-black text-white">
          {elementLabel(raid.boss_element)}
        </span>
        <span className="rounded-pill border border-yellow-300/30 bg-yellow-400/5 px-3 py-1 font-mono text-xs font-black text-yellow-100">
          HP {Number(raid.max_hp).toLocaleString('ko-KR')}
        </span>
      </div>
      <p className="mt-4 max-w-xs text-xs font-bold leading-5 text-cyan-100">
        보스의 외형은 전투 개시와 함께 공개됩니다.
      </p>
    </div>
  );
}
