...
export default function DimensionalGateVN({ character, story, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    void dimensionalGateRpc.startStory(supabase, script.episode_id);
  ...
  useEffect(() => {
    if (!script) return;
    const target = current?.bgm_url ?? (index === 0 ? script.default_bgm_url : undefined);
  ...
  }, [script, current?.cut_order, current?.bgm_url, index]);
  ...
  }, [character.character_id, ended, queryClient, saving, script]);
  ...
  if (scriptQuery.isError || !script || !current) {
    return (
  ...
  const isCg = current.cut_type === 'CG';
    ...
    >
      ...
      <header className="absolute inset-x-0 top-0 z-20 flex items-center gap-3 p-3 sm:p-4" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-[10px] font-black text-white/65">{script.episode_no}편 · {script.title}</div>
      ...
      {isTitle ? (
        ...
        >
          <div>
            <div className="text-xs font-black uppercase tracking-[0.35em] text-violet-100/65">Chapter</div>
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 p-3 pb-5 sm:p-6 sm:pb-8">
          <div className={cn(
            ...
            {!isNarration && current.speaker && <div className="mb-2 text-xs font-black tracking-wide text-violet-200">{current.speaker}</div>}
      ...
      {ended && (
        <div className="absolute inset-0 z-50 grid place-items-center bg-black/88 p-4" onClick={(event) => event.stopPropagation()}>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="max-w-md text-center">
            ...
            <p className="mt-2 text-xs font-semibold leading-relaxed text-white/60">이 장면의 기록이 차원관문에 남았습니다.</p>
            <button type="button" onClick={onClose} className="mt-5 rounded-pill border border-violet-200/25 bg-violet-500/15 px-5 py-2.5 text-xs font-black text-white">차원관문으로 돌아가기</button>
          </motion.div>
        </div>
      )}
