-- B.R.A.N.D 2.0 — student integrated statistics: add actual app-access metrics.
-- Authentication metrics remain available separately for audit/diagnostics.
...
declare
  ...
  v_e jsonb; v_a jsonb; v_x jsonb; v_t jsonb; v_g jsonb; v_r jsonb;
  v_e_student jsonb; v_a_student jsonb; v_char_student jsonb; v_coll_student jsonb;
  v_item_student jsonb; v_att_student jsonb; v_guild_student jsonb;
  v_arcade_summary jsonb; v_arcade_games jsonb;
  v_login_count bigint:=0; v_login_days bigint:=0; v_last_login timestamptz;
  v_access_days bigint:=0; v_access_signals bigint:=0; v_last_access timestamptz;
...
begin
  ...
  if not found then raise exception 'Student not found in active classroom' using errcode='P0620'; end if;
  ...
  select count(*)::bigint, coalesce(sum(a.signal_count),0)::bigint, max(a.last_seen_at)
    into v_access_days,v_access_signals,v_last_access
  from public.app_access_daily a
  where a.classroom_id=p_classroom_id and a.student_id=p_student_id;

  select max(ts) into v_recent_activity
  from (
    select max(a.last_seen_at) as ts from public.app_access_daily a where a.student_id=p_student_id
    union all select max(h.occurred_at) from public.auth_login_history h where h.student_id=p_student_id
  ...
  return jsonb_build_object(
    ...
    'activity',jsonb_build_object(
      'total_access_days',coalesce(v_access_days,0),
      'access_signal_count',coalesce(v_access_signals,0),
      'last_access_at',v_last_access,
      'access_tracking_start_date',(select min(a.access_date) from public.app_access_daily a where a.classroom_id=p_classroom_id),
