...
function loadCredentials() {
  ...
  const lines = fs.readFileSync(CREDS_FILE, 'utf8').replace(/^\uFEFF/, '').split(/\r?\n/).filter(x => x.trim());
...
async function main() {
  ...
  try {
    ...
    const raidId = Number(await rl.question('테스트할 ACTIVE Raid ID: '));
    if (!Number.isInteger(raidId) || raidId <= 0) throw new Error('Raid ID가 올바르지 않습니다.');
    ...
    for (const cred of creds) {
      ...
      try {
        ...
        const state = await getBattleState(auth.client, raidId);
        const raid = state.data?.raid;
        const me = state.data?.me;
        if (!raid || !me) throw new Error('battle state에서 raid/me를 받지 못했습니다. Raid 참가 상태를 확인하세요.');
        ...
        if (message.startsWith('__FATAL_RAID_STATUS__')) {
          ...
          throw new Error(`Raid 상태가 ACTIVE가 아닙니다 (${raidStatus}). 테스트를 시작하지 않았습니다.`);
        }
