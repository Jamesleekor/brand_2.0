...
## 10.1 UI readability hard rule

- 새로 만들거나 수정하는 UI에서 **회색 계열 폰트 색상(`text-text-muted`, `text-text-secondary`, `text-gray-*`, `text-slate-*` 등)을 기본 텍스트 색상으로 사용하지 않는다.**
- 텍스트가 카드/패널/배경 위에 올라갈 때는 색상 선택 전에 **배경 대비와 실제 가독성을 먼저 판단**한다.
- 어두운 배경에서는 아이보리·크림·밝은 베이지 또는 기능 색상처럼 충분한 대비를 확보한 색을 사용한다.
- 비활성/장식 목적이 아닌 제목·라벨·본문·도움말 텍스트는 흐린 회색으로 의미 계층을 만들지 않는다.
- 색상 의미가 있는 정보(속성, 상태, 희귀도)는 해당 의미와 일치하는 색을 사용하되 텍스트 대비를 희생하지 않는다.
- 이 규칙은 B.R.A.N.D 2.0의 신규 UI와 기존 UI를 수정할 때 모두 적용한다.
