...
function RaidGate({
  ...
  const finished = status === 'COMPLETED' || status === 'FAILED' || status === 'ARCHIVED';

  return (
    <div className="absolute left-1/2 top-[6%] z-10 w-[30%] min-w-[280px] max-w-[480px] -translate-x-1/2 text-center">
      <div
        className={cn(
          'relative mx-auto aspect-[0.78] w-full rounded-t-[48%] border-[3px] p-4 transition duration-700',
          active
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-[15%]">
          <div className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-100">
            레이드 관문
          </div>
      ...
      >
        ...
        <div className="mt-1 text-[10px] font-bold text-white">
          {finished
            ? '전투가 종료되었습니다. 결과 화면은 Phase D 이후 연결됩니다.'
            : active
              ? '관문은 활성화되었습니다. 전투 화면은 Phase D에서 연결됩니다.'
              : paused
...
function buildLobbyLayout(players: LobbyPlayer[]): {
  ...
  const groups = new Map<string, RaidLobbyPresence[]>();
