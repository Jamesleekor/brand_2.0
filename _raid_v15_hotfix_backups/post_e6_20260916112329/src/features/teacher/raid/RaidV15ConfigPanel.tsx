...
export default function RaidV15ConfigPanel({ raidId, raidStatus }: { raidId: number; raidStatus: RaidStatus }) {
  ...
  const savePatterns = async () => {
    if (!editableCombat) return;
    const normalized = patterns.map((p, index) => ({ ...p, seq: Number(p.seq || (index + 1) * 10), config: p.config ?? {} }));
    ...
    <section className="rounded-card-lg border border-cyan-400/25 bg-bg-card p-4 shadow-[0_0_28px_rgba(34,211,238,0.04)]">
      ...
      {tab === 'PATTERNS' ? (
        <div className="mt-4 space-y-5">
          <PatternSection title="공통 기믹" description="WEAK_POINT · BREAK · ENRAGE. GROGGY 기본값은 전투 기믹 탭에서 설정합니다." patterns={commonPatterns} allPatterns={patterns} setPatterns={setPatterns} disabled={!editableCombat} />
          <PatternSection title="보스 특수 패턴" description="Boss당 2개 정도를 권장합니다. 필요한 경우 더 추가할 수 있습니다." patterns={specialPatterns} allPatterns={patterns} setPatterns={setPatterns} disabled={!editableCombat} special />
...
function PatternSection({ title, description, patterns, allPatterns, setPatterns, disabled, special = false }: { title: string; description: string; patterns: RaidV15Pattern[]; allPatterns: RaidV15Pattern[]; setPatterns: (v: RaidV15Pattern[]) => void; disabled: boolean; special?: boolean }) {
  ...
  return <div>
    ...
    <div className="space-y-3">
      ...
      {patterns.map((pattern) => <PatternCard key={`${pattern.id ?? 'new'}-${pattern.seq}-${pattern.name}`} pattern={pattern} onChange={(next) => update(pattern, next)} onDelete={() => remove(pattern)} disabled={disabled} common={!special} />)}
    </div>
  </div>;
}

function PatternCard({ pattern, onChange, onDelete, disabled, common }: { pattern: RaidV15Pattern; onChange: (p: RaidV15Pattern) => void; onDelete: () => void; disabled: boolean; common: boolean }) {
  ...
  return <div className={cn('rounded-card-md border p-3', pattern.is_enabled ? 'border-cyan-300/30 bg-bg-deep' : 'border-amber-300/20 bg-black/20 opacity-70')}>
    <div className="grid gap-2 xl:grid-cols-[86px_1.5fr_1.2fr_1.1fr_110px_92px_auto]">
...
function normalizePattern(p: RaidV15Pattern): RaidV15Pattern { return { ...p, pattern_type: p.pattern_type === 'BOSS_STRIKE' ? 'ULTIMATE' : p.pattern_type, config: p.config ?? {} }; }
...
function makePattern(type:RaidV15PatternType,seq:number):RaidV15Pattern { return { seq,name:defaultPatternName(type),pattern_type:type,trigger_kind:'TIME_SECONDS',trigger_value:30,duration_seconds:type==='ABSORB'?4:7,config:defaultPatternConfig(type),is_enabled:true }; }
...
function validatePatterns(ps: RaidV15Pattern[]) {
  ...
  for (const p of ps) {
    ...
    if (p.trigger_kind === 'AFTER_PATTERN' && (!Number.isInteger(p.trigger_value) || p.trigger_value < 1 || p.trigger_value > 999)) { window.alert(`${p.name}: AFTER_PATTERN은 앞 패턴의 Seq(1~999)를 입력합니다.`); return false; }
  }
  return true;
}
